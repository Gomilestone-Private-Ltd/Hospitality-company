        ...
        $('.data-table').DataTable({
                                ...
                                columns:[
                                        ...
                                        {   data: 'status',   name: 'status',
                                            render:function(data,type,row,meta){
                                                $enableBtn = `<p class="changeStatus`+row.slug+`" onclick="return changeStatus('`+row.slug+`','size-status')"><span class="badge badge-success">Enable</span></p>`;
                                                $disableBtn = `<p class="changeStatus`+row.slug+`" onclick="return changeStatus('`+row.slug+`','size-status')"><span class="badge badge-danger">Disable</span></p>`;
                                        ...
                                        {   data: 'action',    name: 'action', 
                                            render:function(data,type,row,meta){
                                                var btn1 = `<a href="{{url('edit-size')}}/`+row.slug+`"> <i class="fa fa-pencil-square edit-icon"  aria-hidden="true"></i></a>`;
                                                var btn2 = `<i class="fa fa-trash delete-icon" aria-hidden="true" onclick="return deleteCategory('`+row.slug+`','delete-size')"></i>`;
